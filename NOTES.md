...
### Interpolation bugs found by comparing with stock (2026-09-10)

- Eruption particle flashes: particles read `Wog::time` during draw; the advanced clock pushed some past their lifetime for one frame. Fix: the clock advances only for the three animation call sites (return addresses 0x14001f45b, 0x14001f499, 0x14002b1b0); everything else sees tick-exact time. Holding the clock in the renderer's effects pass alone was not enough, so that wrapper (`fx_hook`) is redundant but harmless.
- Intro hands flash: restarted keyframe animations produced a negative time. Fix: rate guard 0.15 per tick, clamp at zero, and clock-driven animations (changing within a tick) are left alone.
