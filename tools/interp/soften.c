...
void glyph_soften(u8* px, int size, int r, void* (*alloc)(usz), void (*release)(void*)) {
    ...
    /* pass 1: copy alpha, note whether the glyph has any coverage at all */
    int any = 0;
    for (usz i = 0; i < plane; i++) { u8 a = px[i * 4 + 3]; a0[i] = a; any |= a; }
    if (!any) { release(mem); return; }

    /* pass 2: horizontal box sum per row (edges clamp to 0), h = sum / w */
    for (int y = 0; y < n; y++) {
        const u8* src = a0 + (usz)y * n; u8* dst = h + (usz)y * n;
        int s = 0;
        for (int x = 0; x < r && x < n; x++) s += src[x];
        for (int x = 0; x < n; x++) {
    ...
    for (int x = 0; x < n; x++) col[x] = 0;
    for (int y = 0; y < r && y < n; y++) { const u8* row = h + (usz)y * n; for (int x = 0; x < n; x++) col[x] += row[x]; }
    for (int y = 0; y < n; y++) {
        int add = y + r, rem = y - r - 1;
        if (add < n) { const u8* row = h + (usz)add * n; for (int x = 0; x < n; x++) col[x] += row[x]; }
        if (rem >= 0) { const u8* row = h + (usz)rem * n; for (int x = 0; x < n; x++) col[x] -= row[x]; }
        u8* out = px + (usz)y * n * 4 + 3;
        for (int x = 0; x < n; x++) out[x * 4] = lut[col[x]];
    }
